        ...
        <div class="col-md-6">
            <div class="form-group mb-0 required">
                ...
                <div class="input-group"> 
                    <input name="name" type="text"  id="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{ old('name') ?? $place->name}}" onfocusout="validateName()"> 
                    @if ($errors->has('name'))
            ...
            <div class="form-group mb-0 required">
                ...
                <select name="region"  id="region" class="form-control{{ $errors->has('region') ? ' is-invalid' : '' }}" onfocusout="validateRegiono()">
                        <option class="dropup" value=""> Select One</option>
                         @foreach ($regions as $region)
                       <option class="dropup" value="{{$region->id}}" {{$region->id == $place->region_id ? 'selected' : '' }}> {{$region->name}} </option>  
               @endforeach
