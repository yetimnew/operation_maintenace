		...
		<table class="table table-bordered table-condensed table-striped" id="drivers">
			...
			<tbody>
				...
				<tr>
					...
					<td class='m-1 p-1'><a href="{{route('drivertruck.edit',['id'=> $dt->id])}}" class="btn btn-info btn-sm">Edit</a>
					</td>
					<td class='m-1 p-1'>


						<form action="{{route('drivertruck.detach',['id'=> $dt->id])}}" id="detach-form-{{$dt->id}}" style="display: none">
							@csrf @method('DELETE')
						</form>
						<button type="submit" class="btn btn-danger btn-sm" onclick="if(confirm('Are you sure to detach this?')){
                            ...
                            event.preventDefault();
                        }"> <i class="fas fa-trash"> detach</i></td>
                        </button> @else
						<td class='m-1 p-1'><span class="badge badge-danger">Passive ||</span><span class="pull-right"> Detached {{$dt->date_detach}} </span> </td>
						...
						<td class='m-1 p-1'>
							...
							<form action="{{route('drivertruck.destroy',['id'=> $dt->id])}}" id="delete-form-{{$dt->id}}" style="display: none">
								@csrf @method('DELETE')
							</form>
